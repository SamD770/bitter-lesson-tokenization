"""
Initial test of sequential dependence of gater. 1-3 use a consistency loss rate of 1. 
"""
...
from clean_code.conditional_sequential import SequentiallyDependentRandomGater
import datasets
...
if __name__ == "__main__":
    ...
    model.down_layer_gate = SequentiallyDependentRandomGater(embedding_dim=768, downsample_rate=0.25)
    ...
    train_losses_file = os.path.join(net_scratch_dir, f"experiment_28/train_losses_{args.seed}.csv")
    train_losses.to_csv(train_losses_file, index=False)
    print(f"Train losses saved to {train_losses_file}")


    model_file_name = f"experiment_28/model_{args.seed}.pt"
