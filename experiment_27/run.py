"""
Experiment 27: like experiment_26 but with a lower downsample rate target (0.20) and a higher consistency loss weight (0.1). 
- Prediction: the model will have to learn to gate more tokens.
"""
from clean_code.bitter_llm import set_seed, parameter_count_string
from clean_code.bitter_llm import LinearGater, set_seed, parameter_count_string
from clean_code.flexible_bitter_llm import FlexibleBitterLLM, flexible_training_loop_warm_start 
import datasets
...
if __name__ == "__main__":
    ...
    train_losses = flexible_training_loop_warm_start(
        ...
        batch_limit=20*10**3,
        warm_start_steps=10*10**3,
        learn_gating=True,
        discount_rate=0.9,
        relative_gating_loss_weight=0.01,
        downsample_rate_target=0.20,
    ...
    train_losses_file = os.path.join(net_scratch_dir, f"experiment_27/train_losses_exp27_{args.seed}.csv")
    train_losses.to_csv(train_losses_file, index=False)
    print(f"Train losses saved to {train_losses_file}")

    model_file_name = f"experiment_27/bitter-llm-exp27_{args.seed}.pt"
