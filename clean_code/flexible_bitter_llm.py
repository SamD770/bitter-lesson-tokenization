    ...
    def __init__(
        ...
        if down_layer_gate is None:
            if GaterClass in independent_gater_classes:
                self.down_layer_gate = IndependentWrapperGater(GaterClass(embedding_dim, downsample_rate, **gater_kwargs))
            else:
                self.down_layer_gate = GaterClass(embedding_dim, downsample_rate, **gater_kwargs)
        else:
    ...
    with record_function("backward"):
        ...
        if do_backward_pass:
            try:
                ...
                print(f"error backwarding rank:{accelerator.process_index}: {e}")
                print(f"{out_model['down_gate_samples'].shape=}")
                print(f"{total_loss=} {ar_loss=} {gating_loss=} {down_gate_rate_loss=}")
                print()
                print()
                raise e
            optimizer.step()
