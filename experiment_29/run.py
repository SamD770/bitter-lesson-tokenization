"""
YOLO run of test of sequentially dependent gater with a linear filter prediction. Re-using hyperparameters from experiment 27.
"""
...
if __name__ == "__main__":
    ...
    model.down_layer_gate = SequentiallyDependentLinearGater(embedding_dim=768, downsample_rate=0.25)   
    ...
    train_losses_file = os.path.join(net_scratch_dir, f"experiment_29/train_losses_{args.seed}.csv")
    train_losses.to_csv(train_losses_file, index=False)
    print(f"Train losses saved to {train_losses_file}")


    model_file_name = f"experiment_29/model_{args.seed}.pt"
