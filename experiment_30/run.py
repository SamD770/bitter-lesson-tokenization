"""
YOLO run of test of sequentially dependent gater with a linear filter prediction. Fully re-using hyperparameters from experiment 27.
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Experiment 30 for bitter tokenization")
    ...
    train_losses_file = os.path.join(net_scratch_dir, f"experiment_30/train_losses_{args.seed}.csv")
    train_losses.to_csv(train_losses_file, index=False)
    print(f"Train losses saved to {train_losses_file}")


    model_file_name = f"experiment_30/model_{args.seed}.pt"
